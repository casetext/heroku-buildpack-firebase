...
shell:
	...
	@docker run -e "FIREBASE_TEST_ADMIN_USER=${FIREBASE_ADMIN_USER}" -e "FIREBASE_TEST_ADMIN_PASS=${FIREBASE_ADMIN_PASS}" -v $(shell pwd):/buildpack:ro --rm -it heroku/cedar:14 bash -c 'cp -r /buildpack /buildpack_test; cd /buildpack_test/; bash'
	@echo ""
